...
public class LeetCode0565 {
  ...
  public int arrayNesting(int[] nums) {
    ...
    // Patterns are repeated in cycles. Any numbers cycle are not used in any other
    // cycle, so no reason to recheck ie: A = [5,4,0,3,1,6,2]
    // 1. A[0], A[5], A[6], A[2], A[0]
    // 2. A[2], A[0], A[5], A[6], A[2]
    // 3. A[5], A[6], A[2], A[0], A[5]
    // 4. A[6], A[2], A[0], A[5], A[6]
    boolean[] used = new boolean[nums.length];
    int maxCount = 0;
    for(int i = 0; i < nums.length; i++) {
      // Number been seen in another cycle, skip
      if(used[i]) continue;
      int count = 0;
      int nextIndex = i;
