...
public class LeetCode0690 {
  ...
  public int getImportance(List<Employee> employees, int id) {
    ...
    // Store all employees into HashMap for quicker search by id.
  ...
  private int getImportanceById(int id) {
    if(!map.containsKey(id))
      return 0;
    
    Employee e = map.get(id);
    // Add employee importance
    int sum = e.importance;

    // Now add importance of all of their subordinates
    for(int sub : e.subordinates)
      sum += getImportanceById(sub);
